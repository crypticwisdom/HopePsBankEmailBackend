...
class InitiatorView(APIView):
    ...
    def get(self, request):
        try:
            channel_code: str = request.GET.get("channel", None)
            url:str = f"{settings.IDP_INITIATOR_URL}/oxauth/authorize.htm?scope=contact_info&acr_values=otp&response_type=code" \
    ...
    def get(self, request):
        try:
            ...
            if "code" not in request.GET or not authorization_code:
                return Response({"data": "Authorization code not found."}, status=HTTP_400_BAD_REQUEST)
            ...
            if exchange_auth_code.status_code == 200:
                ...
                if get_bvn_details.status_code != 200:
                    return Response({"detail": f"An error occurred while trying to fetch the resource owner's details. "
                                               f"it returned status_code '{get_bvn_details.status_code}' and message - "
                                               f"'{get_bvn_details.text}'."},
                                    status=HTTP_400_BAD_REQUEST)

                return Response({"detail": get_bvn_details.json()})
            return Response({"detail": f"An error occurred while exchanging an Authorization Code for an Access Token. "
                                       f"Response return status_code - {exchange_auth_code.status_code} with message - "
                                       f"{exchange_auth_code.text}"}, status=HTTP_400_BAD_REQUEST)
        except (Exception, ) as err:
            return Response({"detail": f"{err}"}, status=HTTP_400_BAD_REQUEST)
